...
info = initializeMetadataStruct('tif', fid);
...
%                info(k).DateTime = char(fread(fid, 20, 'uint8')');
            ...
            case 34675
              ...
              case 65053
                % PamGene Evolve CCD Exposure Time (TIFFTAG_EXPOSURETIME)
                fseek(fid, 2, 'cof');
                count = fread(fid, 1, 'uint32');
                if (count <= 4)
                    info(k).ExposureTime = str2num(char(fread(fid, count, 'uint8')'));
                else
                    offset = fread(fid, 1, 'uint32');
                    fseek(fid, offset, 'bof');
                    info(k).ExposureTime = str2num(char(fread(fid, count, 'uint8')'));
            ...
            case 65051
                ...
                count = fread(fid, 1, 'uint32');
                if (count <= 4)
                    info(k).PamchipColumn = str2num(char(fread(fid, count, 'uint8')'));
                else
                    offset = fread(fid, 1, 'uint32');
                    fseek(fid, offset, 'bof');
                    info(k).PamchipColumn = str2num(char(fread(fid, count, 'uint8')'));
              ...
              case 65055
                % PamGene TIFFTAG_INSTRUMENTTYPE 
                fseek(fid, 2, 'cof');
                count = fread(fid, 1, 'uint32');
                if (count <= 4)
                    info(k).PamstationType = char(fread(fid, count, 'uint8')');
                else
                    offset = fread(fid, 1, 'uint32');
                    fseek(fid, offset, 'bof');
                    info(k).PamstationType = char(fread(fid, count, 'uint8')');
            ...
            case 65061
                % PamGene TIFFTAG_UNIT
                fseek(fid, 2, 'cof');
                count = fread(fid, 1, 'uint32');
                if (count <= 4)
                    info(k).PamstationUnit = char(fread(fid, count, 'uint8')');
                else
                    offset = fread(fid, 1, 'uint32');
                    fseek(fid, offset, 'bof');
                    info(k).PamstationUnit = char(fread(fid, count, 'uint8')');
               ...
               case 65052
                % PamGene TIFFTAG_CYCLE 
                fseek(fid, 2, 'cof');
                count = fread(fid, 1, 'uint32');
                if (count <= 4)
                    info(k).Cycle = str2num(char(fread(fid, count, 'uint8')'));
                else
                    offset = fread(fid, 1, 'uint32');
                    fseek(fid, offset, 'bof');
                    info(k).Cycle = str2num(char(fread(fid, count, 'uint8')'));
                  ...
                  case 65050
                % PamGene TIFFTAG_BARCODE 
                fseek(fid, 2, 'cof');
                count = fread(fid, 1, 'uint32');
                if (count <= 4)
                    info(k).PamchipBarcode = str2num(char(fread(fid, count, 'uint8')'));
                else
                    offset = fread(fid, 1, 'uint32');
                    fseek(fid, offset, 'bof');
                    info(k).PamchipBarcode = str2num(char(fread(fid, count, 'uint8')'));
                ...
                count = fread(fid, 1, 'uint32');
                if (count <= 4)
                    info(k).PamchipRow = str2num(char(fread(fid, count, 'uint8')'));
                else
                    offset = fread(fid, 1, 'uint32');
                    fseek(fid, offset, 'bof');
                    info(k).PamchipRow = str2num(char(fread(fid, count, 'uint8')'));
                ...
                case 65054
                % PamGene TIFFTAG_Filter
                fseek(fid, 2, 'cof');
                count = fread(fid, 1, 'uint32');
                if (count <= 4)
                    info(k).Filter = str2num(char(fread(fid, count, 'uint8')'));
                else
                    offset = fread(fid, 1, 'uint32');
                    fseek(fid, offset, 'bof');
                    info(k).Filter = str2num(char(fread(fid, count, 'uint8')'));
