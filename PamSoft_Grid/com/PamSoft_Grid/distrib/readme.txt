    ...
    B. Set the path by doing one of the following:
        ...
        * Add the MCR directory to the environment variable by issuing 
        ...
        NOTE: When deploying C and C++ standalone applications, it is 
              possible to run the shell script file run_PamSoft_Grid.sh 
...
3. Collect the following files, based on what you are deploying: 
    ...
    All Platforms:
        * PamSoft_Grid.ctf (If present)(Component Technology File archive,
          platform-dependent on target machine)

    Windows:
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          installer)
        * PamSoft_Grid.exe (application created by MATLAB Compiler)

    UNIX:
        * MCRInstaller.bin (MATLAB Compiler Runtime installer)
        * PamSoft_Grid (application created by MATLAB Compiler)
        * run_PamSoft_Grid.sh (shell script that can be run to 
    ...
    All Platforms:
        * PamSoft_Grid.dll (shared library, file extension varies by 
          platform)
        * PamSoft_Grid.h (library header file)
        * PamSoft_Grid.ctf (If present)(Component Technology File archive,
    ...
    Windows:
        * PamSoft_Grid.ctf (Component Technology File archive, contains 
          compiled M-code)    
        * PamSoft_Grid.xml (documentation files)
        * PamSoft_Grid.pdb (program database file - if DEBUG option 
          selected)
        * PamSoft_Grid.dll (component assembly file)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
    ...
    Windows:
        * PamSoft_Grid.ctf (Component Technology File archive, contains 
          compiled M-code)
        * PamSoft_Grid_4_1.dll (COM component)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)      
    ...
    All Platforms: 
        * PamSoft_Grid.jar (Java class files and compiled M-code)
    ...
    Windows:
        ...
        * PamSoft_Grid.ctf (Component Technology File archive, contains 
          compiled M-code)
        * PamSoft_Grid.xla (Optionally created and placed in
          C:\PamSoft\DataAnalysis\PamSoft_Grid\com\PamSoft_Grid\src directory by the user)  
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
