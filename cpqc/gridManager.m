...
%---------------------------------------------------------------%
% This is cpqc version of the grid manager !!!!!!!!!!!           %
%---------------------------------------------------------------%
...
handles.oArray = [];
handles.template = 'cpa.mat';
template = load(handles.template);


handles.oArray = template.oArray;
handles.clID = template.clID;
    ...
    function miLoadSet_Callback(hObject, eventdata, handles)
        ...
        if newDir
            ...
            try
                ...
                if ~isequal(handles.instrument, 'PS4_23')
                    error('PamGrid Checkpoints internal QC liscence does not support this instrument.');
                end
           ...
           % Set array object , preprocessing object and segmentation
            ...
            handles.oQ = spotQuantification('backgroundMethod', inipars.qBackgroundMethod, ...
                                            'oOutlier', oOut, 'saturationLimit', 4095);
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
str = ['PamGrid ', handles.version, ' (c)2006 PamGene International BV. PamGene 2 Checkpoints QC tool, internal use only.']; 
helpdlg(str, 'About ...');
...
function exportWell(imNames, aggrName, aggrMode, aggrAxis, oQ, bImages, bSeries)
    ...
    try
        if bImages
            ...
            export(oQ, 'imagesWithSelectedFields', imNames, [], sFields);
        end
        if bSeries
